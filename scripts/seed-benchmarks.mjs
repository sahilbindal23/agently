...
// Load .env.local / .env (Next.js style precedence) so this script works the same
// way next dev does, without requiring a separate dotenv install.
...
for (const f of [".env.local", ".env.development.local", ".env"]) loadEnvFile(f);

const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
if (!url || !key) {
  console.error("Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.");
  console.error("Looked in: process env, .env.local, .env.development.local, .env");
  console.error("Run from the repo root (cwd must contain .env.local).");
  process.exit(1);
}
