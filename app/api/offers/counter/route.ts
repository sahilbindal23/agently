...
export async function POST(request: Request) {
  ...
  if (kind === "deal") return respondToDealCounter(admin, authData.user.id, id, action);
  return respondToProjectCounter(admin, authData.user.id, id, action);
}
...
async function respondToDealCounter(
  ...
  if (!await canManageBrand(admin, profileId, String(deal.brand_id))) return NextResponse.json({ error: "Not allowed to manage this counter." }, { status: 403 });
...
async function respondToProjectCounter(
  ...
  if (!await canManageBrand(admin, profileId, String(project.brand_id))) return NextResponse.json({ error: "Not allowed to manage this counter." }, { status: 403 });
...
async function canManageBrand(admin: NonNullable<ReturnType<typeof createAdminClient>>, profileId: string, brandId: string) {
  const [{ data: profile }, { data: brand }] = await Promise.all([
    admin.from("profiles").select("role").eq("id", profileId).single(),
    admin.from("brands").select("profile_id").eq("id", brandId).single()
  ]);
  if (profile?.role === "admin") return true;
  if (brand?.profile_id === profileId) return true;
