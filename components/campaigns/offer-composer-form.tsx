...
export function OfferComposerForm({
  ...
  const [amountInr, setAmountInr] = useState<number>(defaultAmountInr || 0);
  ...
  return (
    <form className="grid gap-3 md:grid-cols-2" onSubmit={onSubmit}>
      ...
      <Input
        ...
        min="1"
        required
        defaultValue={defaultAmountInr || ""}
        onChange={(event) => setAmountInr(Number(event.target.value) || 0)}
      />
      ...
      <div className="md:col-span-2">
        <FeeBreakdown amountCents={Math.round(amountInr * 100)} audience="brand" />
      </div>
